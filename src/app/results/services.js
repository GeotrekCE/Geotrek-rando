...
function resultsService($rootScope, $q, $location, globalSettings, treksService, sensitiveService, contentsService, eventsService, translationService) {
    ...
    /**
     * Searches a result based on given UID.
     *
     * @param {String} elementUID
     *   The element's UID to seek for.
     *
     * @returns {Promise}
     *   A Promise with found result.
     */
    this.getAResultByUID = function getAResultByUID (elementUID) {
        var deferred = $q.defer();

        var treksSearchRunning = true;
        var contentsSearchRunning = true;
        var eventsSearchRunning = true;

        // Search for treks.
        if (globalSettings.ENABLE_TREKS) {
            treksService.getTreks().then(
                function (treks) {
                    _.forEach(treks.features, function (element) {
                        if (element.uid === elementUID) {
                            deferred.resolve(element);
                        }
                    });
                    treksSearchRunning = false;
                    if (treksSearchRunning === false && contentsSearchRunning === false && eventsSearchRunning === false) {
                        deferred.reject('No matching element found');
                    }
                }
            );
        ...
        // Search for touristic content.
        if (globalSettings.ENABLE_TOURISTIC_CONTENT) {
            contentsService.getContents().then(
                function (touristicContents) {
                    _.forEach(touristicContents.features, function (element) {
                        if (element.uid === elementUID) {
                            deferred.resolve(element);
                        }
                    });
                    contentsSearchRunning = false;
                    if (treksSearchRunning === false && contentsSearchRunning === false && eventsSearchRunning === false) {
                        deferred.reject('No matching element found');
                    }
                }
        ...
        // Search for touristic events.
        if (globalSettings.ENABLE_TOURISTIC_EVENTS) {
            eventsService.getEvents().then(
                function (touriticEvents) {
                    _.forEach(touriticEvents.features, function (element) {
                        if (element.uid === elementUID) {
                            deferred.resolve(element);
                        }
                    });
                    eventsSearchRunning = false;
                    if (treksSearchRunning === false && contentsSearchRunning === false && eventsSearchRunning === false) {
                        deferred.reject('No matching element found');
                    }
                }
